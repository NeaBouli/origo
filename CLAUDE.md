...
# origo — Claude Context

## Was ist das?
Persistent Multiplayer Universe basierend auf Conway's Game of Life. Tausende Spieler teilen sich ein navigierbares Universum, in dem Fraktionen nach Conways Regeln evolvieren. Spieler verdienen GHIFR-Partikel durch Zell-Überleben, austauschbar zu $IFR Tokens via Inferno Protocol (ifrunit.tech). Entwickelt von Vendetta Labs.

## Stack
- **Monorepo**: npm workspaces
- **Server**: Node.js (ESM) + Express + WebSocket (ws) — Port 3000
- **Simulation**: Conway-Engine in JS (server/src/engine/conway.js), Rust-Engine geplant (engine/)
- **Web Client**: React 18 + Three.js + Zustand — Vite Dev-Server Port 5173
- **Datenbank**: PostgreSQL 16 (via pg)
- **Cache**: Redis 7 (via ioredis)
- **Auth**: JWT (jsonwebtoken) + Wallet-Anbindung (ethers.js)
- **Scheduling**: node-cron (Tick-Jobs)
- **Infra**: Docker Compose, Hetzner VPS
- **Node**: >=20.0.0, npm >=10.0.0

## Wichtige Dateipfade
- **Root package.json**: package.json (workspaces, dev/build/migrate Scripts)
- **Server Entry**: server/src/index.js
- **Server Config**: server/.env / server/.env.example
- **Conway Engine**: server/src/engine/conway.js
- **Tick Job**: server/src/jobs/tick.js
- **WebSocket Broker**: server/src/ws/broker.js
- **DB Client**: server/src/db/client.js
- **Redis Client**: server/src/cache/redis.js
- **Core Lib**: packages/core/src/ (grid.js, gameStore.js, wsClient.js, constants.js)
- **Web App Entry**: packages/web/src/main.jsx
- **Web App Root**: packages/web/src/App.jsx
- **Vite Config**: packages/web/vite.config.js
- **DB Migrations**: db/migrations/001_init.sql
- **Migration Runner**: db/migrate.js
- **Docker Compose**: infra/docker-compose.yml
- **Server Dockerfile**: infra/Dockerfile.server
- **Tests**: `npm test` (Jest in server + core)
...
## Aktuelle Phase
Phase 1 — Foundation. Grundstruktur steht (Server, WebSocket, Conway-Engine, Web-Client Skeleton, DB/Redis Infra). Noch kein Renderer-Package (packages/renderer referenziert aber nicht angelegt). Rust-Engine (engine/) noch nicht vorhanden. Mobile (packages/mobile) geplant für Phase 2.

## Kontext-Notizen
- Monorepo mit 3 Workspaces: `packages/*` und `server`
- `@origo/renderer` wird in vite.config.js aliased und im Build-Script referenziert, existiert aber noch nicht als Package
- Vite proxied `/api` → localhost:3000 und `/ws` → ws://localhost:3000
- Docker Compose startet Postgres + Redis + Server als Stack
- Alle Packages nutzen ESM (`"type": "module"`)
- Game-State wird via Zustand verwaltet (packages/core/src/gameStore.js)

## STARTFLOW — Lies das zuerst!
1. Lies dieses `CLAUDE.md` komplett (Stack, Pfade, Do-NOT-touch)
2. Lies `BACKLOG.md` — Aktueller Stand, was funktioniert, was fehlt, nächster Schritt
3. Lies `TODO.md` — Master Task List mit allen Specs und Prioritätsreihenfolge
4. **Server startet NICHT** → Starte bei BUG-001-A (siehe TODO.md)

## Offene Tasks → siehe BACKLOG.md + TODO.md
